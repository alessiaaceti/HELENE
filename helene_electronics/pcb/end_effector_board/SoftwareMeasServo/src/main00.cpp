// ============================================================
//  Helene Robotic Arm – micro-ROS Edition (v2)
...
//     Do not use GPIO 9/10 – they are connected to flash memory!
...
// [3] Forces: written by loop, read by OLED task -> mutex required
...
// In normal operation, connection loss is detected by the ping agent.
void uros_error_handler()
{
    Serial.println("[uROS] Error - reinitializing...");
    ...
    for (;;) {
        if (millis() - millis_timedisp > 100) {
            ...
            if (xSemaphoreTake(g_force_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
                ...
                // Mutex timeout: keep old value, prevent crash
                fx = fy = fz = 0.0f;
            }
        ...
        vTaskDelay(pdMS_TO_TICKS(10)); // Cleaner than yield() under FreeRTOS
    }
}

// ---------- micro-ROS Init / Destroy -------------------------
// [2] Extracted to allow reconnecting during runtime (loop)
...
void uros_destroy()
{
    // Release resources for a clean reconnect
...
void setup()
{
    ...
    Serial.println(F("Helene Robotic Arm - micro-ROS v2"));
    while (l_totalsize > 0) {
    ...
    // Configure ADC (Current configuration: Single channel, Continuous mode)
    obj_ADC1.begin(PIN_ADC1_DRDY, PIN_ADC1_CS);
    obj_ADC1.setMUX(0); // Only reading from MUX 0 (AIN0/AIN1)
    ...
    // CRITICAL FIX: Tell micro-ROS to use Serial2 instead of standard Serial
    set_microros_serial_transports(Serial2);

    // [2] Wait for agent response (LED blinks while waiting)
    Serial.println("[uROS] Waiting for Agent...");
    while (rmw_uros_ping_agent(200, 1) != RMW_RET_OK) {
        digitalWrite(PIN_LED, !digitalRead(PIN_LED));
        delay(100);
    }
    Serial.println("[uROS] Agent found - initializing...");
...
void loop()
{
    // [2] Check connection state - reconnect if lost
    if (!g_uros_connected) {
    ...
    if (g_adc_newdata) {
        ...
        // -----------------------------------------------------------------
        // TEMPORARY FIX: SINGLE-AXIS SENSOR MODE
        // Forces X and Y to 0, assigns raw ADC value to Z axis.
        // Prevents noise from unconnected pins.
        // -----------------------------------------------------------------
        float fx = 0.0f;
        float fy = 0.0f;
        float fz = raw_val; 
        ...
        /* // =================================================================
        // FUTURE FIX: 3-AXIS SENSOR MODE (De-comment when ready)
        // NOTE: You must also change ADC configuration in setup() 
        // to Single-Shot mode instead of Continuous Measurement!
        // =================================================================
        ...
        // Fire Single-Shot conversion here...
        ...
        */
        ...
        // [3] Write forces safely under mutex lock
        ...
        // [1] Debug on Serial0 (USB) - no conflict with micro-ROS on Serial2
    ...
    if (millis() - g_ms_last_publish >= TIME_MS_PUBLISH) {
        ...
        // Read local copy under mutex lock
        if (xSemaphoreTake(g_force_mutex, pdMS_TO_TICKS(2)) == pdTRUE) {
        ...
        if (rcl_publish(&pub_force_x, &msg_force_x, NULL) != RCL_RET_OK ||
            ...
            // Publish error -> mark connection as lost to trigger reconnect
            g_uros_connected = false;
        }
    }

    // 3) Executor spin (handles subscriber callbacks)
